...
describe("secretPublicVars", () => {
  it("finds nothing in an ordinary environment", () => {
    expect(
      secretPublicVars({
        NEXT_PUBLIC_SITE_URL: "https://app.yuvoy.in",
  ...
  it("names every public variable that arrived as the placeholder, sorted", () => {
    expect(
      secretPublicVars({
        ...
        NEXT_PUBLIC_INVITE_ONLY: ` ${SECRET_PLACEHOLDER}\n`,
        NEXT_PUBLIC_SITE_URL: "https://app.yuvoy.in",
      }),
    ).toEqual(["NEXT_PUBLIC_INVITE_ONLY", "NEXT_PUBLIC_POSTHOG_KEY"]);
  });
...
describe("assertNoSecretPublicVars", () => {
  ...
  it("stops the build, naming each variable and the fix", () => {
    ...
    expect(run).toThrow(/NEXT_PUBLIC_INVITE_ONLY, NEXT_PUBLIC_POSTHOG_KEY:/);
...
describe("next.config.ts", () => {
  ...
  it("refuses to load when a public variable arrived as a Secret", async () => {
    vi.stubEnv("NEXT_PUBLIC_INVITE_ONLY", SECRET_PLACEHOLDER);
    await expect(loadConfig()).rejects.toThrow(/^NEXT_PUBLIC_INVITE_ONLY:/);
  });
});
