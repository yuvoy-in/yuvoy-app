...
/**
 * Whether a response came back from this app's own proxy rather than the API.
 *
 * `/api/v1/*` is answered by a Next route handler on THIS server, which then
 * calls the API. So its `Date` is our clock, not the API's, and the two are
 * different machines.
 */
function isProxyResponse(url: string): boolean {
  return safePath(url).startsWith("/api/v1/");
}
...
const errorMiddleware: Middleware = {
  async onResponse({ response, request }) {
    ...
    /*
      ...
      NEVER from the proxy. `createProxyClient` is this same client pointed at
      `/api/v1`, so every proxied call used to overwrite the offset with a
      `Date` generated by the app's own server — which is not the API and not
      the clock the API's `expiresAt` was written against. A hold countdown
      would then be measured against whichever server answered most recently,
      which is a race between two machines rather than a reading of one.

      It is silent in production, where both clocks are roughly right, and
